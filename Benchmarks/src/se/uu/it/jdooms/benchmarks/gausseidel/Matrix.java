...
public class Matrix {
    ...
    private void calculateLeft(int color, Matrix left) {
        for (int row = 1; row < matrix.length - 1; row++) {
            ...
            if ((row + color) % 2 == column % 2) {
                matrix[row][column] = (matrix[row - 1][column] + matrix[row + 1][column]
                        + left.matrix[row][left.matrix[row].length - 1] + matrix[row][column + 1])/4;
            }
        }
    }
    ...
    private void calculateRight(int color, Matrix right) {
        for (int row = 1; row < matrix.length - 1; row++) {
            ...
            if ((row + color) % 2 == column % 2) {
                matrix[row][column] = (matrix[row - 1][column] + matrix[row + 1][column]
                        + matrix[row][column - 1] + right.matrix[row][0])/4;
            }
        }
    }
    ...
    public void calculateMiddle(int color) {
        for (int row = 1; row < matrix.length - 1; row++) {
            for (int column = 1; column < matrix[row].length - 1; column++) {
                if ((row + color) % 2 == column % 2) {
                    matrix[row][column] = (matrix[row - 1][column] + matrix[row + 1][column]
                            + matrix[row][column - 1] + matrix[row][column + 1])/4;
                }
            }
        }
    }
